...
function add_help_buts(lay)
{
	...
	let dx_cont = dx-2*marx-2*marx2-0.4;
	...
	lay.add_title(inter.help.title,1.3,cy-martop + 0.9,{col:WHITE, col_line:WHITE});
	...
	case "import":
		{
			...
			let fo = get_font(1);
			for(let j = 0; j < para.length; j++){
				let w = text_width(j+1);
				if(w > marleft) marleft = w;
			}
			marleft += 0.7;

			for(let j = 0; j < para.length; j++){
				let back_col;
				if(j == inter.help.line){
					back_col = LLBLUE;
					inter.help.liney = cy;
				}
			
				lay.add_paragraph(""+(j+1),dx,0.5,cy,RED,para_si,para_lh);
				
				cy = lay.add_paragraph(para[j],dx-marleft-sep,marleft+sep,cy,BLACK,para_si,para_lh,back_col);
			}
		}
		break;
