...
{ 
	...
	for(auto ti : list){
		...
		if(me.infection_trans){ // If an infection transition then works out specific probability of specific individual
			...
			case 1:
				if(me.ind_variation){
					Li += log(get_indfac(individual[div.ind_trans[0].i],me)*log_thresh(val));	
				}
				else{
					Li += log(log_thresh(val));	
				}
				break;
...
vector <double> StateSpecies::markov_value_calc(unsigned int e, const vector <unsigned int> &list, const vector < vector <double> > &popcomb_t)
{
	...
	if(me.time_vari == false){
		auto value = eq.calculate_param(precalc);
		if(!me.rate) value = 1.0/value;
		
		store.push_back(me_vari.value_t[0]);
		me_vari.value_t[0] = value*dt;
	}		
	else{
		auto &val_t = me_vari.value_t;
		const auto &lin = eq.lin;
		...
		if(lin.on){
			vec = eq.calculate_linear_list(list,popcomb_t,precalc);
				
			if(false){ 
				vector < vector < vector <double> > > derive_val;
				auto vec2 = eq.calculate_para(eq.calcu,list,popcomb_t,precalc,derive_val);
				if(!equal_vec(vec,vec2)) emsg("Vectors are different");
			}
		}
		else{
			if(calc_para_speedup){
				...
				for(auto ti : list){	
					auto value = dt*eq.calculate(ti,popcomb_t[ti],precalc);
					vec.push_back(value);
				}
			}
		}
...
void StateSpecies::markov_value_nopop_restore(const vector <unsigned int> &me_list, const vector <unsigned int> &list, const vector<double> &store)
{
	...
	for(auto ti : list){	
		for(auto e : me_list){
			markov_eqn_vari[e].value_t[ti] = store[m];
			m++;
		}
	}
...
vector <double> StateSpecies::likelihood_nm_trans(unsigned int m, const vector <unsigned int> &list, const vector < vector <double> > &popcomb_t, double &like_ch)
{
	vector <double> store;
	...
	for(auto ti : list){
		...
		if(nmt_er.size() > 0){	
			...
			if(update == UPDATE_TIME){
				for(auto i = 0u; i < ref.size(); i++){
					ref_val[i] = eqn[ref[i]].calculate(ti,popcomb,precalc);
				}
			}
			...
			for(auto &nmti : nmt_er){
				...
				if(update == UPDATE_IND){
					for(auto i = 0u; i < ref.size(); i++){
						ref_val[i] = eqn[ref[i]].calculate_indfac(ind,ti,popcomb,precalc);
					}
				}
...
/// Calculate the likelihood of non-Markovian transitions
vector <double> StateSpecies::likelihood_nm_trans_bp(unsigned int m, const vector <unsigned int> &list, const vector < vector <double> > &popcomb_t, double &like_ch)
{
	vector <double> store;
				...
				if(nm_ref[ti].size() > 0){
					...
					for(auto &nmti : nm_ref[ti]){
						...
						for(auto e : nmt.bp_other_eq){
							prob -= eqn[e].calculate_indfac(ind,ti,popcomb,precalc);
						}
				...
				if(nmt_ref[ti].size() > 0){
					...
					if(update == UPDATE_IND){
						for(auto &nmti : nmt_ref[ti]){
							...
							auto val = eq.calculate_indfac(ind,ti,popcomb,precalc);
							
							if(nmt.all_branches){
								auto div = 0.0;
								for(auto e : nmt.bp_all_eq){
									div += eqn[e].calculate_indfac(ind,ti,popcomb,precalc);
								}
								val /= div;
							}
...
vector <double> StateSpecies::likelihood_nm_trans_incomp(unsigned int n, const vector <unsigned int> &list, const vector < vector <double> > &popcomb_t, double &like_ch)
{
	vector <double> store;
	...
	for(auto ti : list){
		...
		for(auto b = 0u; b < B; b++){
			if(update[b] == UPDATE_TIME){
				...
				const auto &ref = sp.nm_trans[m].dist_param_eq_ref;
				for(auto i = 0u; i < ref.size(); i++){
					ref_val[b][i] = eqn[ref[i]].calculate(ti,popcomb,precalc);
				}
			}
		}
		...
		if(update_bp == UPDATE_TIME){	
			for(auto b = 0u; b < B; b++){
				if(b != b_sel){
					auto m = nmti.nmtrans_ref[b];			
					auto bp_eq = sp.nm_trans[m].bp_eq;
					bp_val[b] = eqn[bp_eq].calculate(ti,popcomb,precalc);
				}
			}
		...
		for(auto i : nmit_er){
			...
			for(auto b = 0u; b < B; b++){
				if(update[b] == UPDATE_IND){	
					...
					const auto &ref = sp.nm_trans[m].dist_param_eq_ref;
					for(auto i = 0u; i < ref.size(); i++){
						ref_val[b][i] = eqn[ref[i]].calculate_indfac(ind,ti,popcomb,precalc);
					}
				}
			}
			...
			if(update_bp == UPDATE_IND){	
				for(auto b = 0u; b < B; b++){
					if(b != b_sel){
						auto m = nmti.nmtrans_ref[b];			
						auto bp_eq = sp.nm_trans[m].bp_eq;			
						bp_val[b] = eqn[bp_eq].calculate_indfac(ind,ti,popcomb,precalc);
					}
				}
...
vector <NMupdate> StateSpecies::likelihood_ie_nm_trans_change(unsigned int i, unsigned int ie, double factor, const vector < vector <double> > &popcomb_t, double &like_ch)
{
	...
	for(auto e = 0u; e < event.size(); e++){
		...
		if(ev.type == NM_TRANS_EV){
			...
			if(sp.tra_ie_affect[ev.tr_gl][ie]){
				...
				for(auto i = 0u; i < ref.size(); i++){
					const auto &eq = eqn[ref[i]];
					ref_val[i] = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);	
				...
				if(sp.tra_gl[trg].branch){                 // Accounts for branching probability 
					...
					if(bp_eq == BP_FROM_OTHERS){
						...
						for(auto e : nmt.bp_other_eq){
							const auto &eq = eqn[e];
							auto value = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);
						...
						if(nmt.all_branches){
							...
							for(auto e: nmt.bp_all_eq){
								const auto &eq = eqn[e];
								auto val = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);
	...
	for(auto j = 0u; j < ind.incomp_ref.size(); j++){
		...
		if(inmt.on){
			...
			auto val = get_nm_incomp_val_ie_factor(ie,factor,nmt,ti,ind,popcomb_t);
...
void StateSpecies::likelihood_pop()
{
	auto list = seq_vec(T);
	for(auto tr = 0u; tr < sp.tra_gl.size(); tr++){
		...
		calculate_tnum_mean_para(tnm,list,tr,cpop_st);
...
void StateSpecies::likelihood_pop_section(unsigned int tr, unsigned int ti1, unsigned int ti2, double &like_ch)
{
...
void StateSpecies::likelihood_pop_spline_section(unsigned int tr, unsigned int ti1, unsigned int ti2, double &like_ch)
{
...
void StateSpecies::likelihood_ib_spline_section(unsigned int ee, unsigned int ti, unsigned int ti_next, const vector < vector <double> > &popcomb_t, double &like_ch)
{
	...
	for(auto tii = ti; tii < ti_next; tii++){
		...
		auto &val_t = me_vari.value_t[tii];
		...
		auto dLi = -(value-val_t)*div.indfac_int;
		
		auto n = div.ind_trans.size();	
		if(n > 0 && !me.infection_trans){
			dLi += n*log(log_thresh(value)/log_thresh(val_t));
		}
		dLi_store.push_back(dLi); value_store.push_back(val_t);
...
double StateSpecies::Li_update_c(unsigned int c, int ma, unsigned int ti, unsigned int ti_next)
{
	auto d = 0.0;
	for(auto tr : sp.comp_gl[c].tr_leave){
		...
		for(auto tii = ti; tii < ti_next; tii++){
			...
			if(c_bef != 0){                              // This is a faster way to calculate
				...
				tnmr = calculate_tnum_mean(tii,tr,cpop_st[tii]);
			}
...
vector <double> StateSpecies::likelihood_pop_change(unsigned int tr, const vector <unsigned int> &list, double &like_ch)
{
	vector <double> store;
...
NMIncompVal StateSpecies::get_nm_incomp_val(const NMTransIncomp &nmti, unsigned int ti, const Individual &ind, const vector < vector <double> > &popcomb_t) const
{
	...
	for(auto b = 0u; b < B; b++){
		...
		for(auto i = 0u; i < ref.size(); i++){
			const auto &eq = eqn[ref[i]];
			if(eq.ind_eff == true) ref_val[b][i] = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);
			else{
				if(eq.time_vari) ref_val[b][i] = eq.calculate(ti,popcomb_t[ti],precalc);
				else{
		...
		if(bp_eq != UNSET){
			if(bp_eq == BP_FROM_OTHERS) b_sel = b;
				...
				const auto &eq = eqn[bp_eq];
				if(eq.ind_eff == true) bp_val[b] = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);
				else{
					if(eq.time_vari) bp_val[b] = eq.calculate(ti,popcomb_t[ti],precalc);
...
NMIncompVal StateSpecies::get_nm_incomp_val_ie_factor(unsigned int ie, double factor, const NMTransIncomp &nmti, unsigned int ti, const Individual &ind, const vector < vector <double> > &popcomb_t) const
{
	const auto &nmtrans_ref = nmti.nmtrans_ref;
	...
	for(auto b = 0u; b < B; b++){
		...
		for(auto i = 0u; i < ref.size(); i++){
			const auto &eq = eqn[ref[i]];
			ref_val[b][i] = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);
		...
		if(bp_eq != UNSET){
			if(bp_eq == BP_FROM_OTHERS) b_sel = b;
				...
				const auto &eq = eqn[bp_eq];
				bp_val[b] = eq.calculate_indfac(ind,ti,popcomb_t[ti],precalc);
...
double StateSpecies::sum_markov_prob(double t1, double t2, unsigned int c, unsigned int tr_gl, unsigned int i, vector < vector <double> > &en) const
{
	...
	if(tr_gl_cor == UNSET) return 0; // This was need for when move event
...
void StateSpecies::calc_trans_diag(ParticleSpecies &ps, const vector < vector <double> > &popcomb_t)
{	
	switch(type){
	case POPULATION: pop_trans_stats(ps); break;
	case DETERMINISTIC: emsg("No diagnostic for deterministic"); break;
	case INDIVIDUAL: ind_trans_stats(ps,popcomb_t,false); break;
	}
}


/// Gets population transition statistics
void StateSpecies::pop_trans_stats(ParticleSpecies &ps) const
{
...
/// Gets individual transition statistics (or probability outside CI if CIprob is set)
double StateSpecies::ind_trans_stats(ParticleSpecies &ps, const vector < vector <double> > &popcomb_t, bool CIprob)
{	
	const auto &precalc = param_val.precalc;
	...
	if(CIprob){    // Accounts for sources
	...
	auto num = 0.0, num_outside_CI = 0.0;
	const auto per_low = 0.05, per_high = 0.95; // The percentile range
			...
			case NM_TRANS_EV:
				{
					...
					if(CIprob){
						auto pr = pr_nm_cpd(tr_gl,ev_or.tdiv,ev.tdiv,popcomb_t,ind);
						num++;
						if(pr < per_low || pr > per_high) num_outside_CI++;
					}
					else{
						add_nm_prob(tr_gl,ev_or.tdiv,ps,popcomb_t,ind);
						add_nm_cpd(tr_gl,ev_or.tdiv,ev.tdiv,ps,popcomb_t,ind);
					}
					...
					if(tlg.markov){
						for(auto tr_gl : tlg.tr_list){
							...
							if(CIprob){
								num++;
								if(pr_max > per_low){
									num_outside_CI += per_low/pr_max;
									if(pr_max > per_high) num_outside_CI += (pr_max-per_high)/pr_max;
								}
								else num_outside_CI++;
							}
							else{
						...
						if(B == 1){
							if(CIprob){
								auto pr = pr_nm_cpd(tlg.tr_list[0],t1,UNSET,popcomb_t,ind);
								num++;
								if(pr < per_low || pr > per_high) num_outside_CI++;
							}
							else{
								add_nm_prob(tlg.tr_list[0],t1,ps,popcomb_t,ind);
								add_nm_cpd(tlg.tr_list[0],t1,UNSET,ps,popcomb_t,ind);
							}
						}
						else{
							auto ti = get_ti(t1);
							...
							for(auto b = 0u; b < B; b++){
								...
								vector <double> ref_val(ref.size());
								for(auto i = 0u; i < ref.size(); i++){
									ref_val[i] = eqn[ref[i]].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
								}
	
								auto bp = eqn[tra.bp.eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
							...
							if(CIprob){
								num++;
								for(auto b = 0u; b < B; b++){
									auto pr_br = val_st[b]/sum2;
									auto pr_max = pr_st[b];
									
									if(pr_max > per_low){
										num_outside_CI += pr_br*per_low/pr_max;
										if(pr_max > per_high) num_outside_CI += pr_br*(pr_max-per_high)/pr_max;
									}
									else num_outside_CI += pr_br;
								}
							}
							else{
								// For future predition of transions rate the branch is randomly selected 
								{
									...
									add_nm_prob(tlg.tr_list[k],t1,ps,popcomb_t,ind);								
								}
	...
	if(CIprob) return num_outside_CI/num;
	return UNSET;
}
			

/// Adds probabilities for a non-Markovian transition
void StateSpecies::add_nm_prob(unsigned tr_gl_or, double t1, ParticleSpecies &ps, const vector < vector <double> > &popcomb_t, const Individual &ind) const
{						
	auto &en = ps.exp_num;
	double t_end = T;
...
/// Gets the probability of a non-Markovian 
double StateSpecies::pr_nm_cpd(unsigned tr_gl_or, double t1, double t2, const vector < vector <double> > &popcomb_t, const Individual &ind) const 
{
	double t_end = T;
	...
	if(t2 != UNSET){
		return nm_trans_incomp_like_no_log(nmt.type,t2-t1,dt,ref_val);
	}
	else{
		return ran()*nm_trans_incomp_like_no_log(nmt.type,t_end-t1,dt,ref_val);
	}
}


/// Adds to cumulative probability distribution
void StateSpecies::add_nm_cpd(unsigned tr_gl_or, double t1, double t2, ParticleSpecies &ps, const vector < vector <double> > &popcomb_t, const Individual &ind) const
{	
	auto &cpd = ps.cum_prob_dist;

	auto pr = pr_nm_cpd(tr_gl_or,t1,t2,popcomb_t,ind);
