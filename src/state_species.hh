...
class StateSpecies                         // Stores information about the state
{
	public:
		...
		ListMove get_ev_link_listmove(unsigned int e, const vector <Event> &event, const Individual &ind, const vector < vector <double> > &popcomb_t) const;
		TRange ev_link_trange(const ListMove &lm, const vector <Event> &event) const;
		bool ev_link(const Event &ev, const Individual &ind, const vector < vector <double> > &popcomb_t) const;
		...
		void get_rate_warning(ParticleSpecies &part_sp) const;
	...
	public:
		void check(unsigned int ti, const vector < vector <double> > &popcomb_t);
	...
	void print_likelihood_markov();
		...
		NMIncompVal get_nm_incomp_val_ie_factor(unsigned int ie, double factor, const NMTransIncomp &nmti, unsigned int ti, const Individual &ind, const vector < vector <double> > &popcomb_t) const;
	...
	// In 'species_update_ind.cc'
		...
		void set_incomp_ref(unsigned int i, const vector < vector <double> > &popcomb_t, double &like_ch);
		void setup_nm_trans(const vector < vector <double> > &popcomb_t);
	...
	// In state_species_local.cc
		...
		void update_rate_mean(const vector < vector <double> > &popcomb_t);
