...
PAS::PAS(const Model &model, Mpi &mpi, Output &output) : model(model), mpi(mpi), output(output), state(model)
{	
...
void PAS::run()
{
	...
	do {
		...
		for (auto s = 0; s < model.nsample_per_gen; s++) { // Iterates over MCMC samples
			...
			for (auto val : mcmc.L_inf_events)
				L += val;
			mcmc.L_samp.push_back(L);

			Sample samp;
			samp.param_value = mcmc.param_value;
			gen_sample.push_back(samp);
		}
		gen_plot(g);

		if (phi == model.phi_final)
			break;
	...
	for (auto s = 0; s < model.nsample; s++) {
		...
		mcmc.update(s);                            // Performs MCMC updates
		...
		if (s % model.nthin == 0) {
			...
			if (mcmc.burnin == false)
				mcmc.store_sample(); // Stores a parameter sample (for statistical analysis later)
		}

		if (model.sample_states > 0) {
			auto step = int(model.nsample / model.sample_states);
			if (s % step == 0) {
				mcmc.output_state(s / step);
				
				if (mcmc.burnin == false && model.ie_output == true) {
					mcmc.store_ie_sample(); 
				}
			}
		}
	...
	if (mpi.core == 0)
		cout << "\nOutputs are placed in the '" << dir << "' directory" << endl;
