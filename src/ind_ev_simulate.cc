...
void IndEvSampler::add_future_nm_event(unsigned int c, unsigned int cl, double t, unsigned int i, vector <FutureNMEvent> &future_nm, unsigned int m, const vector <TrigEventRef> &trig_event)
{
	...
	if(tlg.branch){
		...
		for(auto b = 0u; b < B; b++){
			...
			sum += eqn[tra.bp.eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
			sum_store.push_back(sum);
		}
	...
	case EXP_RATE_NM: 	
		{
			auto rate = eqn[dp[0].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
	...
	case EXP_MEAN_NM: 	
		{
			auto mean = eqn[dp[0].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
	...
	case GAMMA:
		{
			auto mean = eqn[dp[0].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
			auto cv = eqn[dp[1].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
	...
	case ERLANG:
		{
			auto mean = eqn[dp[0].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
			auto shape = eqn[dp[1].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
	...
	case WEIBULL:
		{
			auto scale = eqn[dp[0].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
			auto shape = eqn[dp[1].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
	...
	case PERIOD:
		{
			auto time = eqn[dp[0].eq_ref].calculate_indfac(ind,ti,popcomb_t[ti],precalc);
		...
		do{
			...
			if(tend < t_ev_next){
				...
				if(eve.observed){  // An observed event
					...
					default: emsg("Should not be here6"); break;
					}
				}
