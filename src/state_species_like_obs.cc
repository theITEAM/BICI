...
/*
/// Determines if an individual contains a LI_WRONG 
bool StateSpecies::ind_li_wrong(unsigned int i)
{
	const auto &indi = sp.individual[i];
	const auto &obs = indi.obs;
	const auto &ind = individual[i];
	...
	for(auto k = 0u; k <= ev.size(); k++){
		...
		while(m < obs.size() && obs[m].tdiv <= t){
			...
			switch(ob.type){
			case OBS_TRANS_EV: case OBS_SOURCE_EV: case OBS_SINK_EV: 
				if(obs[m].tdiv != t) return true;
				else{
					if(k < ev.size()){
						const auto &e = ev[k];
						if(e.observed == true){
							if(get_trans_obs_prob(e.tr_gl,obs[m]) == 0) return true;
						}
					}
				}
				break;	
			...
			case OBS_COMP_EV: 
				if(ob.not_alive){ 
					...
					if(c == UNSET) return true;
						...
						if(c_exact != UNSET){
							if(sp.comp_gl[c].cla_comp[ob.cl] != c_exact) return true;
						}
						else{
	...
	if(ent.c_set != UNSET){
		...
		return log(prob);
				...
				Li += like_ind_init_obs(c,indi.enter_ref);
		}	
	}
