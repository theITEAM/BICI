...
{
  short e, k, co, j, ch, c, pp, m, n, fl;
  double prob = 0, t, tt, tst, tnext, tnextold, z, R, r, val, kshape, futnext, tbeg;
  long tr;
  tmp = 1;
  ...
  if(evpbeg == 0){                 // Initialise
    ...
    if(nmfl == 1){ for(k = 0; k < nnonexpc[c]; k++) prob += addfute(nonexpc[c][k],t);}
  }
  ...
  for(e = evpbeg; e < evpend; e++){
    ...
    do{
      while(tnextold <= t){ co = cexist[k]; k++; if(k < nexist) tnextold = texist[k]; else tnextold = tmax;}
      getdep(compleavedep[c],co,t);
      R = Rdep + Rnotdep;
      ...
      if(R == 0) t = tt;
        ...
        if(t > tt) t = tt;
          ...
          if(nmfl == 1){ for(n = 0; n < nnonexptra[tr]; n++)  prob += addfute(nonexptra[tr][n],t);}    // adds on non-Markovian future events
      ...
      if(t == futnext){   // Future event
        ...
        if(inside(c,n,tbeg,pa.pback) == 0){  // Adds the future event
          ...
          c += tra[tr].dc;
          evc.push_back(c); evt.push_back(t);
          for(n = 0; n < nnonexptra[tr]; n++) prob += addfute(nonexptra[tr][n],t);
        }
      }
    }while(t < tnext);
  }
...
short Chain::inside(short c, short n, double tbeg, short pback)                       // Checks if there has been some intervening event in that classification
{
  short e, cnew, cl, p;
  double tsta;

  tsta = futev[n].tnow; cl = futev[n].cl;

  e = evc.size()-1;
  while(e >= 0 && evt[e] > tsta){ cnew = evc[e]; if(cifcl[c][cnew] == cl) return 1; c = cnew; e--;}

  if(tsta < tbeg){
    p = pback;
    do{
      e = part[p].evcst.size()-1;
      while(e >= 0 && part[p].evtst[e] > tsta){ cnew = part[p].evcst[e]; if(cifcl[c][cnew] == cl) return 1; c = cnew; e--;}
      if(e >= 0 && part[p].evtst[e] <= tsta) return 0;
      p = part[p].pback;
    }while(p != -1);
  }
  return 0;
...
double Chain::part_revsec(long i, short evpbeg, short evpend, PART &pa)     // probability of simulating the existing event sequence
{
  ...
  if(evpbeg == 0){                 // initialise
    ...
    if(nmfl == 1){ for(k = 0; k < nnonexpc[c]; k++)  prob += addfuterev(nonexpc[c][k],t);}
  }
  else{            // Does the particle filter
    pp = ppbegst;
    prob += log(part[pp].w/wsum_part);
    ...
    do{
      ...
      if(R == 0){
        ...
        if(erev < nevrev && evrev[erev].t < tt){ // Actual event occurs
          ...
          if(nmfl == 1){ for(n = 0; n < nnonexptra[tr]; n++)  prob += addfuterev(nonexptra[tr][n],t);}    // adds on non-Markovian future events
...
double Chain::addfute(long tr, double t)                                             // Adds a future event
{
  short m;
  double tt, val, kshape, prob = 0;
  FUTEV fev;
  ...
  if(tt < tlea){
    ...
    futeref.insert(futeref.begin()+m,futev.size()); nfuteref++;

    fev.t = tt;
    futev.push_back(fev);

    switch(tra[tr].type){
      case GAMMA_TR: return gammaprob(tt-t,kshape,kshape/val);
      case WEI_TR: return weibullprob(tt-t,val,kshape);
    }
  }
  else{
    switch(tra[tr].type){
      case GAMMA_TR: return gammaup(tlea-t,kshape,kshape/val);
      case WEI_TR: return weibullup(tlea-t,val,kshape);
    }
  }
...
double Chain::addfuterev(long tr, double t)                                             // Adds the future event consistent with exisiting event sequence
{
  short m, e, cl;
...
void Chain::part_init(short exist, short i, double tbirth)          // Initialises a particle proposal
{
  ...
  if(i < nind){ if(nevplist != nindobs[i]+nindfixev[i]+1){cout <<nevplist << " "<<  nindobs[i] << " " << nindfixev[i]+1  <<" uu\n";emsg("wrongh");}}
  else{ if(nevplist != 1) emsg("wrongg");}

  partdt = tmax/50.0; dt = (tlea-tent)/10; if(dt < partdt) partdt = dt;
  cout << nparttot << " nparttot\n";
